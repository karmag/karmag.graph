...
(defn walk
  ...
  will be used for both nodes and links, node and link functions are
  ignored in that case. If either node or link function is nil, and no
  item function is given, the corresponding item type will not
  be iterated.

  An initial state may be passed which will be nil otherwise."
  ([graph {:keys [node-fn link-fn item-fn state]}]
   (let [node-fn (or item-fn node-fn)
         link-fn (or item-fn link-fn)]
